// Slider - Updated February 19, 2025
function noop() { }
function run(fn) {
    return fn();
}
...
// (110:4) {#each slides as slide (slide.url.url)}
function create_each_block(key_1, ctx) {
	let swiper_slide;
	let a;
	let div;
	let raw_value = /*slide*/ ctx[4].content.html + "";
	let t0;
	let img;
	let img_src_value;
	let a_href_value;
	let t1;
	let swiper_slide_key_value;
	...
	return {
		...
		l(nodes) {
			swiper_slide = claim_element(nodes, "SWIPER-SLIDE", { key: true, class: true });
			var swiper_slide_nodes = children(swiper_slide);
			a = claim_element(swiper_slide_nodes, "A", { href: true, class: true, style: true });
			...
			t1 = claim_space(swiper_slide_nodes);
			swiper_slide_nodes.forEach(detach);
			this.h();
		},
		h() {
			attr(div, "class", "content svelte-utfh62");
			if (!src_url_equal(img.src, img_src_value = /*slide*/ ctx[4].image.url)) attr(img, "src", img_src_value);
			attr(img, "class", "svelte-utfh62");
			attr(a, "href", a_href_value = /*slide*/ ctx[4].url.url);
			attr(a, "class", "container svelte-utfh62");
			set_style(a, "--extra_height_mobile", /*slide*/ ctx[4].extra_height);
			set_custom_element_data(swiper_slide, "key", swiper_slide_key_value = /*slide*/ ctx[4].url.url);
			set_custom_element_data(swiper_slide, "class", "svelte-utfh62");
			this.first = swiper_slide;
		},
		m(target, anchor) {
			insert_hydration(target, swiper_slide, anchor);
			append_hydration(swiper_slide, a);
			append_hydration(a, div);
			div.innerHTML = raw_value;
			append_hydration(a, t0);
			append_hydration(a, img);
			append_hydration(swiper_slide, t1);
		},
		p(new_ctx, dirty) {
			ctx = new_ctx;
			if (dirty & /*slides*/ 1 && raw_value !== (raw_value = /*slide*/ ctx[4].content.html + "")) div.innerHTML = raw_value;
			if (dirty & /*slides*/ 1 && !src_url_equal(img.src, img_src_value = /*slide*/ ctx[4].image.url)) {
				attr(img, "src", img_src_value);
			}

			if (dirty & /*slides*/ 1 && a_href_value !== (a_href_value = /*slide*/ ctx[4].url.url)) {
			...
			if (dirty & /*slides*/ 1 && swiper_slide_key_value !== (swiper_slide_key_value = /*slide*/ ctx[4].url.url)) {
				set_custom_element_data(swiper_slide, "key", swiper_slide_key_value);
			}
		},
		d(detaching) {
			if (detaching) detach(swiper_slide);
		}
	};
}
...
function create_fragment(ctx) {
	...
	const get_key = ctx => /*slide*/ ctx[4].url.url;
	...
	return {
		...
		l(nodes) {
			...
			swiper_container = claim_element(div_nodes, "SWIPER-CONTAINER", {
				"auto-height": true,
				navigation: true,
				pagination: true,
				autoplay: true,
			...
			var swiper_container_nodes = children(swiper_container);

			for (let i = 0; i < each_blocks.length; i += 1) {
				each_blocks[i].l(swiper_container_nodes);
			}

			swiper_container_nodes.forEach(detach);
			div_nodes.forEach(detach);
			this.h();
		},
		h() {
			set_custom_element_data(swiper_container, "auto-height", false);
			set_custom_element_data(swiper_container, "navigation", "true");
			set_custom_element_data(swiper_container, "pagination", true);
			set_custom_element_data(swiper_container, "autoplay", true);
			set_style(swiper_container, "--swiper-pagination-color", "black");
			set_style(swiper_container, "--swiper-navigation-color", "#cccccc88");
			set_custom_element_data(swiper_container, "class", "svelte-utfh62");
			attr(div, "class", "swiper-container svelte-utfh62");
		},
		m(target, anchor) {
			insert_hydration(target, div, anchor);
			append_hydration(div, swiper_container);
			...
			if (!mounted) {
				dispose = listen(swiper_container, "swiperslidechange", /*onSlideChange*/ ctx[1]);
				mounted = true;
			}
		},
		p(ctx, [dirty]) {
			if (dirty & /*slides*/ 1) {
				...
				each_blocks = update_keyed_each(each_blocks, dirty, get_key, 1, ctx, each_value, each_1_lookup, swiper_container, destroy_block, create_each_block, null, get_each_context);
			}
		},
		i: noop,
		o: noop,
...
function instance($$self, $$props, $$invalidate) {
	...
	const onSlideChange = e => {
		...
		const currentHeight = swiper.slides[swiper.activeIndex]?.getBoundingClientRect().height;

		if (currentHeight) {
			swiper.el.style.maxHeight = currentHeight + 'px';
		}
	};
	...
	return [slides, onSlideChange, props];
}

class Component extends SvelteComponent {
	constructor(options) {
		super();
		init(this, options, instance, create_fragment, safe_not_equal, { props: 2, slides: 0 });
	}
}
